...
nds cloudsql start
docker run --network cloud_sql_proxy -e "DB_HOST=cloud_sql_proxy:3306" -e "DB_USER=proxyadmin" -e "DB_PASSWORD=" -e "DB_NAME=${projectSlug}-edge"  -e "WP_HOME=https://${projectSlug}-edge.phela.dev" -e "WP_SITEURL=https://${projectSlug}-edge.phela.dev/wp" gcr.io/aleph-infra/nought-wp:v0.0.1 sh -c "/start.sh && wp core install --url=https://${projectSlug}-edge.phela.dev --title=${projectSlug}-edge --admin_user=superuser --admin_email=ping@alephsf.com --admin_password=noughtagoodpassword --allow-root && wp plugin activate wp-migrate-db-pro wp-migrate-db-pro-cli --allow-root"
docker run --network cloud_sql_proxy -e "DB_HOST=cloud_sql_proxy:3306" -e "DB_USER=proxyadmin" -e "DB_PASSWORD=" -e "DB_NAME=${projectSlug}-prs"  -e "WP_HOME=https://${projectSlug}-prs.phela.dev" -e "WP_SITEURL=https://${projectSlug}-prs.phela.dev/wp" gcr.io/aleph-infra/nought-wp:v0.0.1 sh -c "/start.sh && wp core install --url=https://${projectSlug}-prs.phela.dev --title=${projectSlug}-prs --admin_user=superuser --admin_email=ping@alephsf.com --admin_password=noughtagoodpassword --allow-root && wp plugin activate wp-migrate-db-pro wp-migrate-db-pro-cli --allow-root"`
